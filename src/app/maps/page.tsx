...
export default function MapsPage() {
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 font-display flex flex-col justify-between relative overflow-hidden select-none">
      <div className="absolute top-0 right-0 w-[45rem] h-[45rem] bg-rose-900/10 rounded-full blur-3xl -z-10 pointer-events-none" />
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-12 pt-24 pb-16 space-y-8 z-10">
        ...
        <motion.div 
          ...
          className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-stone-800 pb-8"
        >
          <div>
            <div className="inline-flex items-center gap-2 px-3.5 py-1 bg-amber-500/10 border border-amber-500/30 text-amber-400 rounded-full text-xs font-mono font-bold mb-3 shadow-inner">
            ...
            <h1 className="text-3xl md:text-5xl font-black text-white tracking-tight font-display uppercase">
              Campaign <span className="text-rose-400">Worlds &amp; Stages</span>
            </h1>
            <p className="text-xs md:text-sm text-stone-400 mt-2 max-w-2xl leading-relaxed font-mono">
        ...
        <motion.div
          ...
          className="p-6 md:p-8 rounded-3xl bg-gradient-to-r from-stone-900 via-stone-950 to-stone-900 text-white shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 border border-stone-800 backdrop-blur-md"
        >
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <span className="text-4xl p-3 bg-stone-950/90 border border-stone-800 rounded-2xl shadow-inner">
                {activeWorld.icon}
              </span>
              <div>
          ...
          <div className="flex items-center gap-4 bg-stone-950/80 p-4 rounded-2xl border border-stone-800 shrink-0">
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3 rounded-2xl border text-xs font-bold transition-all flex items-center justify-between gap-3 text-left group ${
                      isActive
                        ? 'bg-gradient-to-r from-amber-500/20 to-orange-500/20 text-white border-amber-500/80 ring-2 ring-amber-500/30 shadow-lg'
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span className={`text-xl p-1.5 rounded-xl shrink-0 transition-transform group-hover:scale-110 ${
                        isActive ? 'bg-stone-900 border border-amber-500/50' : 'bg-stone-900 border border-stone-800'
                      }`}>
                        {world.icon}
                      </span>
                      <div className="min-w-0">
                        <span className={`text-[10px] font-mono block ${isActive ? 'text-amber-400' : 'text-stone-500'}`}>
                          WORLD {world.id}
                        </span>
              ...
              {activeWorld.stages.map((stg, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    whileHover={{ y: -4 }}
                    className={`p-6 rounded-3xl border transition-all flex flex-col justify-between backdrop-blur-md shadow-2xl ${
                      isUnlocked
                        ? isBossStage
                          ? 'bg-rose-950/30 border-rose-800/80 hover:border-rose-500 shadow-rose-950/30 ring-1 ring-rose-500/20'
                          : 'bg-stone-900/90 border-stone-800 hover:border-amber-400/80 shadow-black/50'
                        : 'bg-stone-950/50 border-stone-900 opacity-60'
                    }`}
                  >
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-3">
                        <span className="text-xs font-mono font-black text-stone-400 uppercase">
                          STAGE {stg.worldId}-{stg.stageInWorld}
                        </span>
                        <div className="flex items-center gap-1.5">
                          {stg.maps && stg.maps.length > 0 && (
                            <span className="px-2 py-0.5 text-[9px] uppercase font-mono font-bold rounded bg-cyan-950 text-cyan-300 border border-cyan-800 flex items-center gap-1">
                              <Layers className="w-2.5 h-2.5" /> {stg.maps.length} Sub-Areas
                            </span>
                          )}
                          <span className={`px-2.5 py-0.5 text-[9px] uppercase tracking-wider font-mono rounded-md border ${stg.diffClass}`}>
                            {stg.difficulty}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-3xl p-2 bg-stone-950 rounded-2xl border border-stone-800 shadow-inner">
                          {stg.icon}
                        </span>
                        <div>
                          <h3 className="text-xl font-black text-white leading-tight font-display uppercase">
                            {stg.name.replace(/^World \d+-\d+:\s*/, '')}
                          </h3>
                          {isBossStage ? (
                            <span className="text-xs font-bold text-rose-400 block mt-0.5 flex items-center gap-1 font-display">
                              <Skull className="w-3.5 h-3.5" /> Boss: {stg.boss}
                            </span>
                          ) : (
                            <span className="text-xs font-semibold text-stone-500 block mt-0.5 font-mono">
                      ...
                      <p className="mt-4 text-xs text-stone-300 leading-relaxed p-3.5 bg-stone-950/80 border border-stone-800 rounded-2xl font-mono">
                        {stg.description}
                      </p>
                    </div>
                    ...
                    <div className="mt-6">
                      {isUnlocked ? (
                        ...
                        <div className="w-full py-3 bg-stone-950 text-stone-500 rounded-2xl font-bold text-xs border border-stone-800 flex items-center justify-center gap-1.5 cursor-not-allowed font-mono">
